...
if (command == `${p}textmute` || command == `${p}текстмут`) {
// Эмбеды ошибок
...
// Переменные
  ...
  let muterole = message.guild.roles.find(`name`, "💬 › Text Muted");
  let mutetime = args[1];
...
if (command == `${p}voicemute` || command == `${p}голосмут`) {
// Эмбеды ошибок
const notomute = new Discord.RichEmbed()
.setTitle("Ошибка")
.setDescription(`${client.emojis.get(emojis.no)} Данный пользователь не является участником данного сервера.`)
...
if (command == `${p}textunmute` || command == `${p}текстразмут`) {
// Эмбеды ошибок
const noperm = new Discord.RichEmbed()
.setTitle("Ошибка")
.setDescription(`${client.emojis.get(emojis.no)} Вы не являетесь модератором данного сервера.`)
...
if (command == `${p}voiceunmute` || command == `${p}голосразмут`) {
// Эмбеды ошибок
const noperm = new Discord.RichEmbed()
.setTitle("Ошибка")
.setDescription(`${client.emojis.get(emojis.no)} Вы не являетесь модератором данного сервера.`)
